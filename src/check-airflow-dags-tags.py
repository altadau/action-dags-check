...
for dag_file in dag_files:
    ...
    # Обновляем флаги
    emr_tags_found = emr_tags_found or bool(emr_tags)
    tags_found = tags_found or bool(tags)
    sec_conf_found = sec_conf_found or found_sec_conf
...
if not emr_tags_found:    
    print("::error::No EMR Tags found in any of the Airflow DAG files.")
    exit(1)
 
if not tags_found:    
    print("::error::No Airflow Tags found in any of the Airflow DAG files.")    
    exit(1)
 
if not sec_conf_found:    
    print("::error::SecurityConfiguration not found in any of the Airflow DAG files.")
    exit(1)